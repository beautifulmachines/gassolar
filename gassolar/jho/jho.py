...
class Aircraft(Model):
    ...
    def setup(self, Wfueltot, df70=True):
        ...
        Volavn = Variable("\\mathcal{V}_{avn}", 0.125, "ft**3", "avionics volume")
...
class AircraftLoading(Model):
    ...
    def setup(self, aircraft, state, Wcent):
        ...
        hbend = aircraft.emp.tailboom.tailLoad(
            aircraft.emp.tailboom, aircraft.emp.htail, state
        )
        vbend = aircraft.emp.tailboom.tailLoad(
            aircraft.emp.tailboom, aircraft.emp.vtail, state
        )
        ...
        loading.append(TailBoomFlexibility(aircraft.emp.htail, hbend, aircraft.wing))
...
class FlightSegment(Model):
    ...
    def setup(self, N, aircraft, alt=15000, wind=False, etap=0.7):
        ...
        with Vectorize(N):
            ...
            self.slf = SteadyLevelFlight(
                self.fs, self.aircraft, self.aircraftPerf, etap
            )
...
class Mission(Model):
    ...
    def setup(self, wind=False, DF70=True):
        ...
        climb1 = Climb(
            10, self.JHO, alt=np.linspace(0, 15000, 11)[1:], etap=0.508, wind=wind
        )
