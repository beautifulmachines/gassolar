...
def get_highestsens(model, res, varnames=None, N=10):
    ...
    if varnames:
        ...
        for s in res["sensitivities"]["constants"]:
            sens[model[s].key] = sum(np.hstack([res["sensitivities"]["constants"][s]]))
    ...
    sensdict = {"positives": pss, "negatives": ngs, "indicies": ind, "labels": labels}
    return sensdict
...
def plot_chart(sensdict):
    ...
    ax.bar(sensdict["indicies"], -np.array(sensdict["negatives"]), 0.5, color="#3FBAC2")
...
def test():
    ...
    vn = {
        model.solar.Wpay: "$W_{\\mathrm{pay}}$",
