...
def jho_subs(model):
    ...
    subs = {
        model.JHO.wing.mfac: sol(model.JHO.wing.mfac),
        model.JHO.emp.mfac: sol(model.JHO.emp.mfac),
        "Mission.Aircraft.Fuselage.m_{fac}": sol("Mission.Aircraft.Fuselage.m_{fac}"),
    }
    model.substitutions.update(subs)
...
def perf_solve(model):
    ...
    vloiter = np.average(sol("Mission.Loiter.FlightSegment.FlightState.V").magnitude)
...
def max_speed(model):
    ...
    oldcost = model.cost
    model.cost = 1.0 / np.prod(model["Mission.Loiter.FlightSegment.FlightState.V"])
...
def plot_climbrate(result):
    fig, ax = plt.subplots()
    ax.plot(
        result("h.Mission.Climb.FlightSegment.FlightState").magnitude - 1500,
        result("\\dot{h}"),
        "k",
    )
...
def plot_glide(result):
    LoD = np.mean(
        np.hstack([result(l) / result(d) for l, d in zip(result("CL"), result("C_D"))])
    )
    h = (
        result("h.Mission.Climb.FlightSegment.FlightState")
