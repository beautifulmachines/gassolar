...
def plot_solarlat(df):
    ...
    a = (df["80"][i80] - df["95"][i95]) / (df["latitude"][i80] - df["latitude"][i95])
    b = df["95"][i95] - a * df["latitude"][i95]
    c = a * df["latitude"][i95 + 1 : i80 + 1] + b
    df["95"][i95 + 1 : i95 + 1 + len(c)] = c
    ax.fill_between(
        df["latitude"], df["80"], df["95"], alpha=0.3, facecolor="b", edgecolor="None"
    )
    ...
    labels = [
        "$\\pm$%d" % l
        for l in np.linspace(min(df["latitude"]), max(df["latitude"]), len(labels))
    ]
