...
for av in [85, 90, 95]:
    for l in [35, 40, 45]:
        ...
        M.substitutions.update({"f_{structures}": ("sweep", np.linspace(0.2, 0.5, 10))})
        M.substitutions.update({"h_{batt}": ("sweep", np.linspace(250, 400, 10))})
        ...
        fig.savefig(
            "../../../gassolarpaper/bcontourl%da%d.pdf" % (l, 85), bbox_inches="tight"
        )
