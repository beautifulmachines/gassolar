...
def fit_setup(altitude=(40000, 80000), latitude=45, day=355):
    ...
    for p in percentiles:
        wind.append(np.array(get_windspeed(latitude, p, altitude, day)) / WIND_NORM)
    ...
    for l in latrange:
        ...
        for j, a in enumerate(altitudestart):
            ...
            for K in range(2, 6):
                for ftype in ["MA", "SMA"]:
                    try:
                        ...
                        print("Fit failed: Lat %d; K = %d; ftype = %s;") % (l, K, ftype)
                        err = 0.9
                    if err < rms_best:
            ...
            if rms_best == 1:
                ...
                if gen:
                    df_best.to_csv(
                        "windfits" + month + "/windaltfit_lat%d.csv" % l, index=False
                    )
                if plot:
