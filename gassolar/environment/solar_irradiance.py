...
def twi_fits(latitude, day, gen=False):
    ...
    if not gen:
        ...
        ax1.set_ylabel(
            "Twilight Energy $(E/S)_{\\mathrm{twilight}}}$" + " [Whr/m$^2$]",
            fontsize=19,
        )
        ax1.grid()
    ...
    if not gen:
        ...
        ax2.grid()
        ax2.set_xlabel(
            "Minimum Necessary Power $(P/S)_{\\mathrm{min}}$" + " [W/m$^2$]",
            fontsize=19,
        )
        ax2.set_ylabel(
            "Daytime Energry $(E/S)_{\\mathrm{day}}$ [Whr/m$^2$]", fontsize=19
        )
...
if __name__ == "__main__":
    ...
    if not GENERATE:
        ...
        fig, ax = plt.subplots()
        ax.fill_between(
            [-12, -td / 2], 0, 80, alpha=0.3, facecolor="b", linewidth=2, color="b"
        )
        ax.fill_between(
            [td / 2, 12], 0, 80, alpha=0.3, facecolor="b", linewidth=2, color="b"
        )
        ax.fill_between(
            p[1],
            p[0],
        ...
        ax.fill_between(
            p[1], 0, p[0], hatch="xx", color="r", linewidth=2, facecolor="none"
        )
        newp = []
        for pp in p[0]:
            if pp < 80:
                ...
                newp.append(80)
        ax.fill_between(p[1], 0, newp, linewidth=2, color="g", facecolor="g", alpha=0.3)
        ...
        ax.annotate(
            "$(E/S)_{\\mathrm{twilight}}$",
            xy=(-4.9, 70),
            xytext=(-10.1, 220),
            arrowprops=dict(
                facecolor="black", shrink=0.05, width=1.5, headwidth=10, frac=0.1
            ),
        )
        ax.annotate(
            "",
            ...
            xytext=(-6.7, 205),
            arrowprops=dict(
                facecolor="black", shrink=0.05, width=1.5, headwidth=10, frac=0.025
            ),
        )
        ax.grid()
        ax2 = ax.twinx()
