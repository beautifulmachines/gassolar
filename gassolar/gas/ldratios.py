...
def ld_plot(model, num):
    ...
    model.substitutions.update(
        {"N_{max}_Mission/AircraftLoading/WingLoading/GustL": 0.1}
    )
    ...
    if num == 1 or num == 2:
        ...
        l = ax.plot(
            clm, cdm, "o", mfc="None", ms=7, mew=1.5, label="With Wind Constraint"
        )
        ax.annotate(
            "mission start",
            xy=(clm[0], cdm[0]),
    ...
    if num == 2:
        l = ax.plot(
            clw, cdw, "s", mfc="None", ms=7, mew=1.5, label="Without Wind Constraint"
        )
        ax.annotate(
            "mission start",
            xy=(clw[0], cdw[0]),
    ...
    l = ax.plot(
        cl, cd, linewidth=2, label="Re=%3.fk" % (re / 1000.0), c="#084081", zorder=1
    )
    lines = [l[0]]
    re = sol("Re_Mission/Loiter/FlightSegment/AircraftPerf/WingAero")
    for i, r, col in zip(
        range(5), re, ["#0868ac", "#0868ac", "#0868ac", "#2b8cbe", "#2b8cbe"]
    ):
        cd = return_cd(cl, r)
        cl15 = cl**1.5 / cd
        clmax = cl[cl15 == max(cl15)]
    ...
    labelLines(
        lines, fontsize=12, zorder=[2.5] * 3, align=False, xvals=[1.05, 0.85, 0.7]
    )
    ax.set_xlabel("$C_L$")
    ax.set_ylabel("$c_{d_p}$")
    if num == 2:
        ax.legend(
            ["With Wind Constraint", "Without Wind Constraint"], fontsize=15, loc=2
        )
    if num == 1:
