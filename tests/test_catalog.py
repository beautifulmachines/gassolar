...
def test_catalog_ir_roundtrip(model_entry):
    ...
    when the round-trip diff is non-empty. The latter is a known gap
    (gpkit-core#214): from_ir() drops constant-only variables that are never
    referenced by a constraint or the cost, which every gassolar model hits
    via constants-only material submodels like CFRPFabric.
    """
    if ir_diff is None:
    ...
    if diff:
        pytest.skip(f"{cls.__name__} IR round-trip hit gpkit-core#214:\n{diff}")
